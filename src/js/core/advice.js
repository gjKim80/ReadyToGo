 ...
 * @returns {Array<{id,icon,text,tone}>}
 */
export function buildAdvice(weather, destWeather = null, plan = null) {
  ...
  if (weather.sky === "snow") {
    tips.push({ id: "snow", icon: "snow", tone: "cold", text: "눈 예보 — 미끄럼 방지 신발과 이동 시간 여유를 두세요." });
  }
  ...
  if (weather.tempMin <= 0) {
    tips.push({ id: "freeze", icon: "scarf", tone: "cold", text: `최저 ${Math.round(weather.tempMin)}℃ — 목도리와 장갑을 챙기세요.` });
  }

  if (weather.tempMax >= 31) {
    tips.push({ id: "heat", icon: "cold-drink", tone: "warn", text: `최고 ${Math.round(weather.tempMax)}℃ — 물과 양산, 그늘 이동을 권장합니다.` });
  }

  if ((weather.windMs ?? 0) >= 8) {
    tips.push({ id: "wind", icon: "wind", tone: "warn", text: `순간 풍속 ${weather.windMs}m/s — 우산보다 우비가 안전합니다.` });
  }
  ...
  if (!tips.length) {
    tips.push({ id: "clear", icon: "thumbs-up", tone: "info", text: "특별히 챙길 건 없어요. 가볍게 다녀오세요!" });
  }
